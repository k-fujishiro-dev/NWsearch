...
struct CSVExporter {
    func export(session: TestSession) -> [URL] {
        ...
        let dateString = ISO8601DateFormatter().string(from: session.createdAt)

        let summaryURL = tempDir.appendingPathComponent("session_summary_\(dateString).csv")
        let endpointURL = tempDir.appendingPathComponent("endpoint_detail_\(dateString).csv")
