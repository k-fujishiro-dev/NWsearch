...
struct ResultSummaryView: View {
        ...
        if let session = sessionStore.session(id: sessionID) {
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    if hasMissingMetrics(session.summary) {
                        Text("一部指標は測定不能です。")
                            .font(.footnote)
    ...
    private func hasMissingMetrics(_ summary: SummaryMetrics) -> Bool {
        summary.medianMs == nil || summary.jitterMs == nil || summary.dnsIndexMs == nil || summary.downMbps == nil
    }
}
